...
def main():
    ...
    for result in results:
        for i in range(result[1]):
            itemList.append(textGame.TextCard(result[0]))
    ...
    for result in results:
        for i in range(result[1]):
            eventList.append(textGame.TextCard(result[0], True))

    db.close()

    items = textGame.TextDeck(sys_random, itemList)
    events = textGame.TextDeck(sys_random, eventList, True)

    playSpace = textGame.TextBoard(sys_random)

    players = [textGame.TextPlayer(0, playSpace.get(1, 0), sys_random, "Gabe"),
               textGame.TextPlayer(0, playSpace.get(1, 2), sys_random, "Other")]

    players = rollSort(players)
    ...
    for player in reversed(players):
        ...
        for j in range(playSpace.width):
            if playSpace.grid[0][j] is None or playSpace.grid[0][j].occupied is not None:
                pass
            else:
                options.append(playSpace.grid[0][j])

        textGame.displayAll(textGame.giveOptions(options))

        choice = textGame.getInput(player.name + ", choose a starting location: ", range(len(options)))

        playSpace.grid[player.location.y][player.location.x].occupied = None
        player.location = options[choice]
        options[choice].occupied = player

    for player in players:
        player.draw(items, events, playSpace, players)

    index = 0
    while True:
        temp = players[index].turn(players, playSpace, items, events)
        if temp[0]:
            print(players[index].name + " won!")
            break
        else:
            index = temp[1]
